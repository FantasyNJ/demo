...
function createCal(y, m){  //m是自然月
    ...
    if(date.getDay() == 0){  //如果第一天是星期天
        var num = -5;
    }else{                   //如果第一天不是星期天
        var num = 2 - getWeek(date.getFullYear(), date.getMonth());  // 计数判断 -2 -1 0 1 2 .... 31 32 33
    }
...
//获取生成月份的行数，m是程序的月份（自然月-1）
function getLine(y, m){
    var d = new Date(y, m,1);
    if(d.getDay() == 0){   //如果1号是星期天，那么前面有6天
        return Math.ceil((6 + getDays(y, m)) / 7);
    }else{   //如果1号不是星期天，那么前面有星期几-1天
        return Math.ceil((d.getDay() - 1 + getDays(y, m)) / 7);
    }
}
//获取月份一号是星期几
